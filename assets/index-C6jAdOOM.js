//#region src/lib/FileSystemAccessAPIStorageProvider.tsx
var FileSystemAccessAPIStorageProvider = class {
	...
	****************/
	...
	#jobPromiseQueue = [];
	async #scheduleJob() {
		if (this.#jobsInProgress < this.#MAX_OPEN_FILES) this.#jobsInProgress++;
		else {
	...
	#declareJobDone() {
		if (this.#jobPromiseQueue.length > 0) this.#jobPromiseQueue.shift().resolve();
		else this.#jobsInProgress--;
	}
	async #getSubFolderHandle(folderHandle, subDirName) {
		return await folderHandle.getDirectoryHandle(subDirName, { create: true });
	}
	async #getDescendantFolderHandle(folderHandle, descendantFolderPath) {
		if (!descendantFolderPath) return folderHandle;
		if (this.#descendantFolderHandles.has(descendantFolderPath)) return this.#descendantFolderHandles.get(descendantFolderPath);
		const pathSegments = descendantFolderPath.length > 0 ? this.#splitPath(descendantFolderPath) : [];
		let dirHandle = folderHandle;
		for (const pathSegment of pathSegments) dirHandle = await this.#getSubFolderHandle(dirHandle, pathSegment);
	...
	async #getDescendantFileHandle(folderHandle, filePath, create) {
		...
		return await (folderPathSegments.length > 0 ? await this.#getDescendantFolderHandle(folderHandle, folderPathSegments.join(this.DS)) : folderHandle).getFileHandle(filename, { create });
	}
	async #getFileHandle(requestPath, create) {
		return await this.#getDescendantFileHandle(this.#directoryHandle, requestPath, create);
	}
	/** **************
	PUBLIC
	****************/
	...
	async writeObject(requestPath, data) {
		...
		try {
			const writable = await (await this.#getFileHandle(requestPath, true)).createWritable();
	...
	async renameObject(oldRequestPath, newRequestPath) {
		if (oldRequestPath.substring(0, oldRequestPath.lastIndexOf("/")) === newRequestPath.substring(0, newRequestPath.lastIndexOf("/"))) {
			const fileHandle = await this.#getFileHandle(oldRequestPath, true);
			const newEntryName = newRequestPath.substring(newRequestPath.indexOf("/") + 1);
			await fileHandle.move(newEntryName);
		} else {
			await this.writeObjectFromReadable(newRequestPath, await this.getReadableStream(oldRequestPath));
	...
	async writeObjectFromReadable(requestPath, readableStream) {
		...
		try {
			...
			return await this.getObjectSize(requestPath);
		} finally {
			this.#declareJobDone();
		}
	}
	async readObjectAsString(requestPath) {
		await this.#scheduleJob();
		try {
			return await (await (await this.#getFileHandle(requestPath, false)).getFile()).text();
		} finally {
			this.#declareJobDone();
		}
	}
	async getReadableStream(requestPath, _range) {
		return (await (await this.#getFileHandle(requestPath, false)).getFile()).stream();
	}
	async removeObject(requestPath) {
		const folderPath = requestPath.substring(0, requestPath.lastIndexOf(this.DS));
		const dir = await this.#getDescendantFolderHandle(this.#directoryHandle, folderPath);
		const filename = requestPath.substring(requestPath.lastIndexOf(this.DS) + 1);
		await dir.removeEntry(filename);
	}
	async listSubDirectories(requestPath) {
		const dirHandle = await this.#getDescendantFolderHandle(this.#directoryHandle, requestPath);
		const values = [];
		for await (const handle of dirHandle.values()) values.push(handle);
		return values.filter((value) => value.kind === "directory").map((dirHandle) => dirHandle.name.normalize("NFC"));
	}
	async #getFilenamesInFolder(folderPath) {
		const dirHandle = await this.#getDescendantFolderHandle(this.#directoryHandle, folderPath);
		const filenames = [];
		for await (const handle of dirHandle.values()) if (handle.kind === "file") filenames.push(handle.name.normalize("NFC"));
		else {
			const normalizedDirName = handle.name.normalize("NFC");
			const requestPaths = (await this.#getFilenamesInFolder(this.#joinPath(folderPath, normalizedDirName))).map((filename) => {
	...
	async getObjectSize(requestPath) {
		return (await (await this.#getFileHandle(requestPath, false)).getFile()).size;
	}
	async #getFolderSize(folderPath) {
		const folderHandle = await this.#getDescendantFolderHandle(this.#directoryHandle, folderPath);
		let sum = 0;
		for await (const handle of folderHandle.values()) if (handle.kind === "file") {
			const fileSize = (await handle.getFile()).size;
			sum += fileSize;
		} else {
			const folderSize = await this.#getFolderSize(this.#joinPath(folderPath, handle.name.normalize("NFC")));
			sum += folderSize;
		}
		return sum;
	}
...
const getRandomKey = (collection) => {
	...
	let cntr = 0;
	for (const key of collection.keys()) if (cntr++ === index) return key;
	return null;
};
...
const getCurrentISODateTime = () => {
	return toISODateTime(/* @__PURE__ */ new Date());
};
...
const sluggifyFilename = (text) => {
	return trimSlug(text.normalize("NFC").trim().replace(/['’]+/g, "").replace(/[^\p{L}\p{M}\d\-._]+/gu, "-").replace(/-+/g, "-").replace(/^\./g, "").toLowerCase());
};
...
const createSlug = (noteContent, existingSlugs) => {
	let slugStem = sluggifyNoteText(getNoteTitle(noteContent));
	let n = 1;
	if (!slugStem) slugStem = "new";
	while (true) {
		const slug = slugStem === "new" || n > 1 ? `${slugStem}-${n}` : slugStem;
...
const getSlugAndNameForNewArbitraryFile = (namespace, originalFilename, existingSlugs) => {
	...
	const sluggifiedFileStem = sluggifyFilename(removeExtensionFromFilename(originalFilename));
	let n = 1;
	while (true) {
		const stemWithOptionalIntegerSuffix = n > 1 ? `${sluggifiedFileStem}-${n}` : sluggifiedFileStem;
...
const getLastSlugSegment = (slug) => {
	...
	if (posOfLastSlash > -1) return slug.substring(posOfLastSlash + 1);
	else return slug;
};
//#endregion
//#region src/lib/subwaytext/serialize.ts
...
function serialize(blocks) {
	return blocks.map((block) => {
		switch (block.type) {
			case BlockType.PARAGRAPH: return serializeParagraph(block);
			case BlockType.HEADING: return serializeHeading(block);
			case BlockType.UNORDERED_LIST_ITEM: return serializeUnorderedListItem(block);
			case BlockType.ORDERED_LIST_ITEM: return serializeOrderedListItem(block);
			case BlockType.CODE: return serializeCodeBlock(block);
			case BlockType.QUOTE: return serializeQuote(block);
			case BlockType.KEY_VALUE_PAIR: return serializeKeyValuePair(block);
			case BlockType.EMPTY: return serializeEmpty(block);
		}
	}).join("\n");
}
//#endregion
//#region src/lib/notes/indexUtils.ts
...
const updateBacklinksIndex = (graph, ourSlug, ourOutgoingLinks) => {
	let ourBacklinks;
	if (graph.indexes.backlinks.has(ourSlug)) ourBacklinks = graph.indexes.backlinks.get(ourSlug);
	else {
	...
	for (const someExistingSlug of graph.notes.keys()) {
		if (someExistingSlug === ourSlug) continue;
		if (ourOutgoingLinks.includes(someExistingSlug)) graph.indexes.backlinks.get(someExistingSlug).add(ourSlug);
		else graph.indexes.backlinks.get(someExistingSlug).delete(ourSlug);
		...
		const theirOutgoingLinks = graph.indexes.outgoingLinks.get(someExistingSlug);
...
const updateBlockIndex = (graph, existingNote) => {
	...
	graph.indexes.blocks.set(existingNote.meta.slug, blocks);
	return blocks;
};
...
const updateIndexes = (graph, existingNote) => {
	const ourOutgoingLinks = updateOutgoingLinksIndex(graph, existingNote, updateBlockIndex(graph, existingNote));
...
const serializeNote = (note) => {
	...
	if (note.meta.createdAt) headersToSerialize.set(CanonicalNoteHeader.CREATED_AT, note.meta.createdAt.toString());
	if (note.meta.updatedAt) headersToSerialize.set(CanonicalNoteHeader.UPDATED_AT, note.meta.updatedAt.toString());
	if (note.meta.flags.length > 0) headersToSerialize.set(CanonicalNoteHeader.FLAGS, note.meta.flags.join(","));
	for (const key in note.meta.additionalHeaders) if (Object.hasOwn(note.meta.additionalHeaders, key)) headersToSerialize.set(key, note.meta.additionalHeaders[key]);
...
const getOutgoingLinksToOtherNotes = (graph, slug) => {
	if (!graph.indexes.outgoingLinks.has(slug)) throw new Error("Could not determine outgoing links of " + slug);
...
const getAliasesOfSlug = (graph, slug) => {
	return new Set(Array.from(graph.aliases.entries()).filter((entry) => {
		return entry[1] === slug;
	}).map((entry) => {
		return entry[0];
	}));
...
const getNotePreview = (graph, slug) => {
	if (!graph.notes.has(slug)) throw new Error("Could not generate note preview of " + slug);
...
const getBacklinks = (graph, slug) => {
	...
	if (!backlinkSlugs) throw new Error("Could not determine backlinks for slug " + slug);
	return Array.from(backlinkSlugs).map((slug) => {
		const note = graph.notes.get(slug);
		return {
			slug: note.meta.slug,
...
const getFileSlugsReferencedInNote = (graph, noteSlug) => {
	const allReferencedSlugs = getSlugsFromInlineText(getAllInlineSpans(graph.indexes.blocks.get(noteSlug)));
...
const getFileInfosForFilesLinkedInNote = (graph, slugOfNote) => {
	...
	return new Set(fileSlugs.values().map((fileSlug) => graph.files.get(fileSlug)));
};
...
const createNoteToTransmit = async (existingNote, graph, includeParsedContent) => {
	...
	const noteToTransmit = {
		...
		outgoingLinks: Array.from(getOutgoingLinksToOtherNotes(graph, existingNote.meta.slug)).map((slug) => {
			return getNotePreview(graph, slug);
		}),
	...
	if (includeParsedContent) noteToTransmit.parsedContent = blocks;
	return noteToTransmit;
};
const mapInlineSpans = (blocks, mapper) => {
	return blocks.map((block) => {
		if (block.type === BlockType.PARAGRAPH) block.data.text = block.data.text.map(mapper);
		else if (block.type === BlockType.HEADING) block.data.text = block.data.text.map(mapper);
		else if (block.type === BlockType.QUOTE) block.data.text = block.data.text.map(mapper);
		else if (block.type === BlockType.ORDERED_LIST_ITEM) block.data.text = block.data.text.map(mapper);
		else if (block.type === BlockType.UNORDERED_LIST_ITEM) block.data.text = block.data.text.map(mapper);
		else if (block.type === BlockType.KEY_VALUE_PAIR) block.data.value = block.data.value.map(mapper);
		return block;
	});
};
const getNoteFeatures = (note, graph) => {
	const blocks = graph.indexes.blocks.get(note.meta.slug);
	const containsWeblink = getAllInlineSpans(blocks).some((span) => span.type === SpanType.HYPERLINK);
	const containsCode = blocks.some((block) => block.type === BlockType.CODE);
	let containsImages = false;
	let containsDocuments = false;
	let containsAudio = false;
	let containsVideo = false;
	getFileInfosForFilesLinkedInNote(graph, note.meta.slug).forEach((fileInfo) => {
		const mediaType = getMediaTypeFromFilename(fileInfo.filename);
		if (mediaType === MediaType.IMAGE) containsImages = true;
		else if (mediaType === MediaType.PDF) containsDocuments = true;
		else if (mediaType === MediaType.AUDIO) containsAudio = true;
		else if (mediaType === MediaType.VIDEO) containsVideo = true;
	});
	return {
		containsWeblink,
		containsCode,
...
const createNoteListItems = (existingNotes, graph) => {
	return existingNotes.map((existingNote) => {
		return createNoteListItem(existingNote, graph);
	});
};
const getURLsOfNote = (noteContent) => {
	return [...noteContent.matchAll(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&//=,]*)/g)].map((match) => match[0]);
};
const changeSlugReferencesInNote = (content, oldSlug, newSlug, newSluggifiableTitle) => {
	return mapInlineSpans(content, (span) => {
		if (span.type === SpanType.SLASHLINK && span.text.substring(1) === oldSlug) span.text = "/" + newSlug;
		else if (span.type === SpanType.WIKILINK && sluggifyWikilinkText(span.text.substring(2, span.text.length - 2)) === oldSlug) span.text = "[[" + (newSluggifiableTitle ?? newSlug) + "]]";
		return span;
	});
};
const getSlugsFromParsedNote = (note) => {
	return getSlugsFromInlineText(getAllInlineSpans(note));
};
const handleExistingNoteUpdate = async (noteSaveRequest, io) => {
	...
	if (existingNote === null) throw new Error(ErrorMessage.NOTE_NOT_FOUND);
	...
	if (noteSaveRequest.aliases) {
		for (const [alias, canonicalSlug] of graph.aliases.entries()) if (canonicalSlug === existingNote.meta.slug && !noteSaveRequest.aliases.has(alias)) {
			graph.aliases.delete(alias);
			if (!("changeSlugTo" in noteSaveRequest && typeof noteSaveRequest.changeSlugTo === "string" && noteSaveRequest.changeSlugTo === alias)) aliasesToUpdate.add(alias);
		}
		noteSaveRequest.aliases.forEach((alias) => {
			if (!isValidNoteSlug(alias)) throw new Error(ErrorMessage.INVALID_ALIAS);
			if (alias === existingNote.meta.slug) {
				if (!canonicalSlugShouldChange) throw new Error(ErrorMessage.ALIAS_EXISTS);
			} else if (graph.notes.has(alias)) throw new Error(ErrorMessage.SLUG_EXISTS);
			if (graph.aliases.has(alias) && graph.aliases.get(alias) !== existingNote.meta.slug) throw new Error(ErrorMessage.ALIAS_EXISTS);
			if (graph.aliases.has(alias) && graph.aliases.get(alias) === existingNote.meta.slug) return;
	...
	if ("changeSlugTo" in noteSaveRequest && typeof noteSaveRequest.changeSlugTo === "string") {
		if (!isValidNoteSlug(noteSaveRequest.changeSlugTo)) throw new Error(ErrorMessage.INVALID_SLUG);
		if (graph.notes.has(noteSaveRequest.changeSlugTo)) throw new Error(ErrorMessage.SLUG_EXISTS);
		if (graph.files.has(noteSaveRequest.changeSlugTo)) throw new Error(ErrorMessage.SLUG_EXISTS);
		if (graph.aliases.has(noteSaveRequest.changeSlugTo)) throw new Error(ErrorMessage.ALIAS_EXISTS);
		...
		let flushPins = false;
		for (let i = 0; i < graph.pinnedNotes.length; i++) if (graph.pinnedNotes[i] === oldSlug) {
			graph.pinnedNotes[i] = newSlug;
			flushPins = true;
		}
		const aliasesToUpdate = /* @__PURE__ */ new Set();
		for (const [alias, canonicalSlug] of graph.aliases.entries()) if (canonicalSlug === oldSlug) {
			graph.aliases.delete(alias);
			graph.aliases.set(alias, newSlug);
			aliasesToUpdate.add(alias);
		}
		await io.flushChanges(graph, flushPins, new Set([oldSlug]), aliasesToUpdate, /* @__PURE__ */ new Set());
		...
		if ("updateReferences" in noteSaveRequest && noteSaveRequest.updateReferences) {
			...
			for (const thatNote of notesReferencingOurNoteBeforeChange) {
				const blocks = graph.indexes.blocks.get(thatNote.meta.slug);
				const noteTitle = getNoteTitle(existingNote.content);
				const newBlocks = changeSlugReferencesInNote(blocks, oldSlug, newSlug, sluggifyNoteText(noteTitle) === newSlug ? noteTitle : newSlug);
				thatNote.content = serialize(newBlocks);
				graph.indexes.blocks.set(thatNote.meta.slug, newBlocks);
				updateIndexes(graph, thatNote);
				await io.flushChanges(graph, flushPins, new Set([thatNote.meta.slug]), /* @__PURE__ */ new Set(), /* @__PURE__ */ new Set());
			}
		}
	} else graph.notes.set(existingNote.meta.slug, existingNote);
	updateIndexes(graph, existingNote);
	await io.flushChanges(graph, false, new Set([existingNote.meta.slug]), aliasesToUpdate, /* @__PURE__ */ new Set());
	return await createNoteToTransmit(existingNote, graph);
};
...
const handleNewNoteSaveRequest = async (noteSaveRequest, io) => {
	const graph = await io.getGraph();
	const noteFromUser = noteSaveRequest.note;
	const existingSlugs = [...Array.from(graph.notes.keys()), ...Array.from(graph.aliases.keys())];
	let slug;
	if ("changeSlugTo" in noteSaveRequest && typeof noteSaveRequest.changeSlugTo === "string") {
		if (!isValidNoteSlug(noteSaveRequest.changeSlugTo)) throw new Error(ErrorMessage.INVALID_SLUG);
		if (graph.notes.has(noteSaveRequest.changeSlugTo) || graph.aliases.has(noteSaveRequest.changeSlugTo)) throw new Error(ErrorMessage.SLUG_EXISTS);
		if (graph.files.has(noteSaveRequest.changeSlugTo)) throw new Error(ErrorMessage.SLUG_EXISTS);
		slug = noteSaveRequest.changeSlugTo;
	} else slug = createSlug(noteFromUser.content, existingSlugs);
	const aliasesToUpdate = /* @__PURE__ */ new Set();
	noteSaveRequest.aliases?.forEach((alias) => {
		if (!isValidNoteSlug(alias)) throw new Error(ErrorMessage.INVALID_ALIAS);
		if (graph.aliases.has(alias) && graph.aliases.get(alias) !== slug) throw new Error(ErrorMessage.ALIAS_EXISTS);
		if (graph.notes.has(alias)) throw new Error(ErrorMessage.SLUG_EXISTS);
	...
	await io.flushChanges(graph, false, new Set([newNote.meta.slug]), aliasesToUpdate, /* @__PURE__ */ new Set());
	return await createNoteToTransmit(newNote, graph);
};
//#endregion
//#region src/lib/subwaytext/index.ts?worker&url
var subwaytext_default = "/neno/assets/index-C8ucraal.js";
//#endregion
//#region src/lib/notes/DatabaseIO.ts
var DatabaseIO = class DatabaseIO {
	#storageProvider;
	#loadedGraph = null;
	...
	static getSubtextGraphFilenameForSlug(slug) {
		if (slug.length === 0) throw new Error("Cannot get filename for empty slug");
		return `${slug}${DatabaseIO.#GRAPH_FILE_EXTENSION}`;
	}
	static getSlugFromGraphFilename(filename) {
		if (!filename.endsWith(DatabaseIO.#GRAPH_FILE_EXTENSION)) throw new Error("Filename does not end with default note filename extension");
	...
	static parsePinsFile(pinsSerialized) {
		if (pinsSerialized.length === 0) return [];
		return pinsSerialized.split("\n");
	}
	async getGraphFilenamesFromStorageProvider() {
		return (await this.#storageProvider.getAllObjectNames()).filter((filename) => {
	...
	async parseGraph(serializedGraphFiles, pinsSerialized) {
		...
		for (const [slug, serializedGraphFile] of serializedGraphFiles) try {
			const headers = parseGraphFileHeaders(cleanSerializedNote(serializedGraphFile));
			if (headers.has(DatabaseIO.#ALIAS_HEADER_KEY)) {
				...
				const fileInfo = {
					slug,
					size: parseInt(headers.get(DatabaseIO.#ARBITRARY_FILE_SIZE_HEADER_KEY)),
					filename: headers.get(DatabaseIO.#ARBITRARY_FILE_HEADER_KEY),
					createdAt: headers.get("created-at")
				};
				files.set(slug, fileInfo);
			} else {
				const parsedNote = parseSerializedExistingGraphFile(serializedGraphFile, slug);
		...
		let pinnedNotes;
		if (typeof pinsSerialized === "string") pinnedNotes = DatabaseIO.parsePinsFile(pinsSerialized);
		else {
			pinnedNotes = [];
			await this.writePinsFile(pinnedNotes);
		}
		const blockIndex = await DatabaseIO.createBlockIndex(Array.from(parsedNotes.values()));
		const outgoingLinkIndex = DatabaseIO.createOutgoingLinkIndex(blockIndex);
		const backlinkIndex = DatabaseIO.createBacklinkIndex(outgoingLinkIndex, new Set(parsedNotes.keys()), aliases);
		return {
			notes: parsedNotes,
			aliases,
			files,
	...
	async readAndParseGraphFromDisk() {
		let pinsSerialized;
		try {
			pinsSerialized = await this.#storageProvider.readObjectAsString(DatabaseIO.#PINS_FILENAME);
		} catch (_e) {
			pinsSerialized = void 0;
		}
		const noteFilenames = await this.getGraphFilenamesFromStorageProvider();
		const serializedNotes = new Map(await Promise.all(noteFilenames.map(async (filename) => {
			return [DatabaseIO.getSlugFromGraphFilename(filename), await this.#storageProvider.readObjectAsString(filename)];
		})));
	...
	static createBacklinkIndex(outgoingLinks, existingNoteSlugs, aliases) {
		...
		for (const [slug, links] of outgoingLinks) {
			if (!backlinkIndex.has(slug)) backlinkIndex.set(slug, /* @__PURE__ */ new Set());
			for (const link of links) {
				if (existingNoteSlugs.has(link)) {
					if (!backlinkIndex.has(link)) backlinkIndex.set(link, /* @__PURE__ */ new Set());
					backlinkIndex.get(link).add(slug);
				}
				if (aliases.has(link)) {
					const canonicalSlug = aliases.get(link);
					if (!backlinkIndex.has(canonicalSlug)) backlinkIndex.set(canonicalSlug, /* @__PURE__ */ new Set());
	...
	static createBlockIndex(notes) {
		...
		if (DatabaseIO.#workerPool.length === 0) for (let t = 0; t < concurrency; t++) {
			const worker = new Worker(subwaytext_default, { type: "module" });
		...
		return new Promise((resolve, reject) => {
			...
			for (let t = 0; t < concurrency; t++) {
				...
				worker.onmessage = (event) => {
					...
					for (const noteParsed of notesParsed) blockIndex.set(noteParsed.id, noteParsed.parsedContent);
	...
	async writePinsFile(pins) {
		await this.#storageProvider.writeObject(DatabaseIO.#PINS_FILENAME, pins.join("\n"));
	}
	/**
	PUBLIC
	**/
	...
	async getRawNote(slug) {
		const rawNote = await this.#storageProvider.readObjectAsString(DatabaseIO.getSubtextGraphFilenameForSlug(slug));
		if (!rawNote) throw new Error(ErrorMessage.GRAPH_NOT_FOUND);
		return rawNote;
	}
	async getGraph(forceDiskRead = false) {
		if (this.#graphRetrievalInProgress) await this.#graphRetrievalInProgress;
	...
	async flushChanges(graph, flushPins, canonicalNoteSlugsToFlush, aliasesToFlush, arbitraryFilesToFlush) {
		...
		if (canonicalNoteSlugsToFlush instanceof Set) await Promise.all(Array.from(canonicalNoteSlugsToFlush).map(async (slug) => {
			const filename = DatabaseIO.getSubtextGraphFilenameForSlug(slug);
			if (!graph.notes.has(slug)) await this.#storageProvider.removeObject(filename);
			else await this.#storageProvider.writeObject(filename, serializeNote(graph.notes.get(slug)));
		}));
		else for (const [slug, note] of graph.notes) {
			const filename = DatabaseIO.getSubtextGraphFilenameForSlug(slug);
			await this.#storageProvider.writeObject(filename, serializeNote(note));
		}
		if (aliasesToFlush instanceof Set) await Promise.all(Array.from(aliasesToFlush).map(async (alias) => {
			const filename = DatabaseIO.getSubtextGraphFilenameForSlug(alias);
			if (!graph.aliases.has(alias)) await this.#storageProvider.removeObject(filename);
			else {
				const canonicalSlug = graph.aliases.get(alias);
				await this.#storageProvider.writeObject(filename, serializeNoteHeaders(new Map([[DatabaseIO.#ALIAS_HEADER_KEY, canonicalSlug]])));
			}
		}));
		else for (const [alias, canonicalSlug] of graph.aliases) {
			const filename = DatabaseIO.getSubtextGraphFilenameForSlug(alias);
			await this.#storageProvider.writeObject(filename, serializeNoteHeaders(new Map([[DatabaseIO.#ALIAS_HEADER_KEY, canonicalSlug]])));
		}
		if (arbitraryFilesToFlush instanceof Set) await Promise.all(Array.from(arbitraryFilesToFlush).map(async (slug) => {
			const sgfFilepath = DatabaseIO.getSubtextGraphFilenameForSlug(slug);
			if (!graph.files.has(slug)) await this.#storageProvider.removeObject(sgfFilepath);
			else {
				const fileInfo = graph.files.get(slug);
				const sizeHeaderValue = fileInfo.size.toString();
				const noteHeaders = new Map([[DatabaseIO.#ARBITRARY_FILE_HEADER_KEY, fileInfo.filename], [DatabaseIO.#ARBITRARY_FILE_SIZE_HEADER_KEY, sizeHeaderValue]]);
				if (fileInfo.createdAt) noteHeaders.set(CanonicalNoteHeader.CREATED_AT, fileInfo.createdAt?.toString());
				if (fileInfo.updatedAt) noteHeaders.set(CanonicalNoteHeader.UPDATED_AT, fileInfo.updatedAt?.toString());
				const data = serializeNoteHeaders(noteHeaders);
				await this.#storageProvider.writeObject(sgfFilepath, data);
			}
		}));
		else for (const [slug, fileInfo] of graph.files) {
			const filename = DatabaseIO.getSubtextGraphFilenameForSlug(slug);
			const size = fileInfo.size;
			const data = serializeNoteHeaders(new Map([[DatabaseIO.#ARBITRARY_FILE_HEADER_KEY, fileInfo.filename], [DatabaseIO.#ARBITRARY_FILE_SIZE_HEADER_KEY, size.toString()]]));
			await this.#storageProvider.writeObject(filename, data);
		}
		if (flushPins) await this.writePinsFile(graph.pinnedNotes);
	}
	async addFile(slug, source) {
		if (!isValidSlug(slug)) throw new Error(ErrorMessage.INVALID_SLUG);
		return await this.#storageProvider.writeObjectFromReadable(slug, source);
	}
	async moveArbitraryGraphFile(oldSlug, newSlug) {
		if (oldSlug !== newSlug) await this.#storageProvider.renameObject(oldSlug, newSlug);
	}
	...
	async getReadableArbitraryGraphFileStream(slug, filename, range) {
		...
		return await this.#storageProvider.getReadableStream(filepath, range);
	}
	async getFileSize(slug) {
		if (!isValidSlug(slug)) throw new Error(ErrorMessage.INVALID_SLUG);
		return await this.#storageProvider.getObjectSize(slug);
	}
	...
	async graphExistsInStorage() {
		return (await this.getGraphFilenamesFromStorageProvider()).length > 0;
	}
};
//#endregion
//#region src/lib/notes/types/NoteListSortMode.ts
let NoteListSortMode = /* @__PURE__ */ function(NoteListSortMode) {
...
//#endregion
//#region src/lib/notes/graphUtils.ts
...
const getSortFunction = (sortMode) => {
	const sortFunctions = {
		...
		[NoteListSortMode.TITLE_DESCENDING]: (a, b) => {
			const aNormalized = getSortKeyForTitle(a.title);
			return getSortKeyForTitle(b.title).localeCompare(aNormalized);
		},
...
const getNoteSortFunction = (sortMode) => {
	...
	return sortFunctions.get(sortMode) ?? sortFunctions.get(NoteListSortMode.UPDATE_DATE_ASCENDING);
};
const breadthFirstSearch = (nodes, links, root) => {
	...
	while (queue.length > 0) {
		...
		const connectedNodes = links.filter((link) => {
			...
			return nodes.find((n) => n.meta.slug === linkedNoteId);
		}).filter((n) => {
			return n !== void 0;
		});
...
const getGraphLinks = (graph) => {
	return Array.from(graph.notes.keys()).reduce((links, slug) => {
		if (!graph.indexes.outgoingLinks.has(slug)) throw new Error("Could not determine outgoing links for " + slug);
		const linksFromThisSlug = Array.from(graph.indexes.outgoingLinks.get(slug)).filter((targetSlug) => {
			return graph.notes.has(targetSlug) || graph.aliases.has(targetSlug);
		}).map((targetSlug) => {
			return [slug, graph.aliases.get(targetSlug) ?? targetSlug];
		});
		return [...links, ...linksFromThisSlug];
	}, []);
};
const getNumberOfComponents = (graph) => {
	...
	while (totallyDiscovered.length < nodes.length) {
		...
		totallyDiscovered = [...totallyDiscovered, ...inComponent];
		numberOfComponents++;
		i++;
	}
...
const getGraphCreationTimestamp = (graph) => {
	return getEarliestISOTimestamp(...Array.from(graph.notes.values()).map((note) => note.meta.createdAt).filter((createdAt) => {
		return typeof createdAt === "string";
	}));
};
const getGraphUpdateTimestamp = (graph) => {
	return getLatestISOTimestamp(...Array.from(graph.notes.values()).map((note) => note.meta.updatedAt).filter((createdAt) => {
		return typeof createdAt === "string";
	}));
};
//#endregion
//#region src/lib/notes/searchUtils.ts
const getNotesWithDuplicateUrls = (notes) => {
	const urlIndex = /* @__PURE__ */ new Map();
	notes.forEach((note) => {
		getURLsOfNote(note.content).forEach((url) => {
			if (urlIndex.has(url)) urlIndex.get(url).add(note);
			else urlIndex.set(url, new Set([note]));
		});
	});
	const duplicates = /* @__PURE__ */ new Set();
	for (const notesWithUrl of urlIndex.values()) if (notesWithUrl.size > 1) notesWithUrl.forEach((note) => {
		duplicates.add(note);
	});
...
const getNotesWithDuplicateTitles = (notes) => {
	...
	notes.forEach((note) => {
		...
		if (titleIndex.has(noteTitle)) titleIndex.get(noteTitle).add(note);
		else titleIndex.set(noteTitle, new Set([note]));
	});
	const duplicates = /* @__PURE__ */ new Set();
	for (const notesWithOneTitle of titleIndex.values()) if (notesWithOneTitle.size > 1) notesWithOneTitle.forEach((note) => {
		duplicates.add(note);
	});
	return Array.from(duplicates);
};
...
const getNotesWithFile = (notes, graph, fileSlug) => {
	return notes.filter((note) => {
		return getFileSlugsReferencedInNote(graph, note.meta.slug).has(fileSlug);
	});
};
...
const getNotesWithTitleSlugOrAliasContainingToken = (notes, token, caseSensitive, aliases) => {
	...
	for (const [alias, target] of aliases.entries()) if (caseSensitive && alias.includes(token) || alias.includes(token.toLowerCase())) fittingNoteSlugs.add(target);
	return Array.from(notes).filter((note) => {
		if (token.length === 0) return true;
		if (fittingNoteSlugs.has(note.meta.slug)) return true;
		if (caseSensitive) return getNoteTitle(note.content).includes(token) || note.meta.slug.includes(token);
		else return getNoteTitle(note.content).toLowerCase().includes(token.toLowerCase()) || note.meta.slug.toLowerCase().includes(token.toLowerCase());
	});
};
...
const getNotesWithMediaTypes = (notes, graph, requiredMediaTypes, everyNoteMustContainAllMediaTypes) => {
	return everyNoteMustContainAllMediaTypes ? notes.filter((note) => {
		...
		return setsAreEqual(requiredMediaTypes, new Set(files.values().map((file) => getMediaTypeFromFilename(file.filename))));
	}) : notes.filter((note) => {
		const files = getFileInfosForFilesLinkedInNote(graph, note.meta.slug);
		const includedMediaTypes = new Set(files.values().map((file) => getMediaTypeFromFilename(file.filename)));
...
const getNotesWithLinkToSlug = (notes, graph, fileSlug) => {
	if (!graph.indexes.backlinks.has(fileSlug)) return [];
	return notes.filter((note) => {
...
//#endregion
//#region src/lib/notes/search.ts
const isWhiteSpace = (string) => {
	return string.trim().length === 0;
};
const parseToken = (token) => {
	if (token.length > 1 && token.indexOf(":\"") > -1 && token.lastIndexOf("\"") === token.length - 1) return [token.substring(0, token.indexOf(":\"")), token.substring(token.indexOf(":\"") + 2, token.length - 1)];
	else if (token[0] === "\"" && token[token.length - 1] === "\"") return ["", token.substring(1, token.length - 1)];
	else if (token.includes(":")) {
		const pos = token.indexOf(":");
		return [token.substring(0, pos), token.substring(pos + 1)];
	} else return ["", token];
};
const getRawTokensFromQueryString = (queryString) => {
	...
	while (true) {
		...
		if (mode === "whitespace") if (isWhiteSpace(value)) continue;
		else {
			if (value === "\"") withinQuote = !withinQuote;
			mode = "token";
			collector += value;
		}
		else if (mode === "token") {
...
const search = async (graph, query) => {
	...
	for (let t = 0; t < tokens.length; t++) {
		...
		if (key === "duplicates") if (value === "url") matchingNotes = getNotesWithDuplicateUrls(matchingNotes);
		else if (value === "title") matchingNotes = getNotesWithDuplicateTitles(matchingNotes);
		else matchingNotes = [];
		else if (key === "exact") matchingNotes = getNotesByTitle(matchingNotes, value, false);
		else if (key === "has-url") matchingNotes = getNotesWithUrl(matchingNotes, value);
		else if (key === "has-file") matchingNotes = getNotesWithFile(matchingNotes, graph, value);
		else if (key === "links-to") matchingNotes = getNotesWithLinkToSlug(matchingNotes, graph, value);
		else if (key === "has-flag") matchingNotes = getNotesWithFlag(matchingNotes, value);
		else if (key === "has-block") {
			const types = value.split("|");
			matchingNotes = getNotesWithBlocksOfTypes(matchingNotes, graph, types, false);
		} else if (key === "has-media") {
			const types = value.split("|");
			matchingNotes = getNotesWithMediaTypes(matchingNotes, graph, new Set(types), false);
		} else if (key === "ft") matchingNotes = getNotesThatContainTokens(matchingNotes, value, caseSensitive);
		else if (key.startsWith("$")) matchingNotes = getNotesWithKeyValue(matchingNotes, graph, key.substring(1), value);
		else matchingNotes = getNotesWithTitleSlugOrAliasContainingToken(matchingNotes, value, caseSensitive, graph.aliases);
	}
	if ([
		...
		if (limit > 0 && limit < matchingNotes.length) matchingNotes = matchingNotes.slice(0, limit);
		return {
			results: createNoteListItems(getPagedMatches(matchingNotes, page, 50), graph),
			numberOfResults: matchingNotes.length
		};
	} else return {
		results: getPagedMatches(createNoteListItems(matchingNotes, graph).sort(getSortFunction(sortMode)), page, 50),
		numberOfResults: matchingNotes.length
	};
};
//#endregion
//#region src/lib/notes/index.ts
var NotesProvider = class {
	...
	async get(slug, options) {
		...
		if (!graph.notes.has(canonicalSlug)) throw new Error(ErrorMessage.NOTE_NOT_FOUND);
		return await createNoteToTransmit(graph.notes.get(canonicalSlug), graph, options?.includeParsedContent);
	}
	async getRandom() {
		const graph = await this.#io.getGraph();
		const noteFromDB = graph.notes.size > 0 ? graph.notes.get(getRandomKey(graph.notes)) : null;
		if (!noteFromDB) throw new Error(ErrorMessage.NOTE_NOT_FOUND);
		return await createNoteToTransmit(noteFromDB, graph);
	}
	async getRawNote(slug) {
		slug = slug.normalize("NFC");
		const canonicalSlug = (await this.#io.getGraph()).aliases.get(slug) || slug;
		return this.#io.getRawNote(canonicalSlug);
	}
	async getNotesList(query) {
		return search(await this.#io.getGraph(), query);
	}
	async getStats(options) {
		...
		if (options.includeMetadata) stats.metadata = {
	...
	async put(noteSaveRequest) {
		const noteFromUser = noteSaveRequest.note;
		if (!noteFromUser || typeof noteFromUser.content !== "string") throw new Error(ErrorMessage.INVALID_NOTE_STRUCTURE);
		if (isExistingNoteSaveRequest(noteSaveRequest)) return handleExistingNoteUpdate(noteSaveRequest, this.#io);
		else return handleNewNoteSaveRequest(noteSaveRequest, this.#io);
	}
	async remove(slug) {
		slug = slug.normalize("NFC");
		const graph = await this.#io.getGraph();
		if (!graph.notes.has(slug)) throw new Error(ErrorMessage.NOTE_NOT_FOUND);
		graph.notes.delete(slug);
		const aliasesToRemove = /* @__PURE__ */ new Set();
		for (const [alias, canonicalSlug] of graph.aliases.entries()) if (canonicalSlug === slug) {
			graph.aliases.delete(alias);
			aliasesToRemove.add(alias);
		}
		let flushPins = false;
		graph.pinnedNotes = graph.pinnedNotes.filter((s) => {
			if (s === slug) flushPins = true;
			return s !== slug;
		});
		removeSlugFromIndexes(graph, slug);
		await this.#io.flushChanges(graph, flushPins, new Set([slug]), aliasesToRemove, /* @__PURE__ */ new Set());
	}
	#fileAdditionInProgress = null;
	#finishedAddingFile = () => {};
	async addFile(readable, namespace, originalFilename) {
		if (this.#fileAdditionInProgress) await this.#fileAdditionInProgress;
		...
		try {
			const graph = await this.#io.getGraph();
			const { slug, filename } = getSlugAndNameForNewArbitraryFile(namespace, originalFilename, getAllUsedSlugsInGraph(graph));
			const fileInfo = {
				slug,
				filename,
				size: await this.#io.addFile(slug, readable),
				createdAt: getCurrentISODateTime(),
				updatedAt: getCurrentISODateTime()
			};
			graph.files.set(slug, fileInfo);
			await this.#io.flushChanges(graph, false, /* @__PURE__ */ new Set(), /* @__PURE__ */ new Set(), new Set([slug]));
			return fileInfo;
		} finally {
			this.#finishedAddingFile();
		}
	...
	async updateFile(readable, slug) {
		...
		const graph = await this.#io.getGraph();
		const fileInfo = graph.files.get(slug);
		if (!fileInfo) throw new Error(ErrorMessage.FILE_NOT_FOUND);
		fileInfo.size = await this.#io.addFile(slug, readable);
		await this.#io.flushChanges(graph, false, /* @__PURE__ */ new Set(), /* @__PURE__ */ new Set(), new Set([slug]));
		return fileInfo;
	}
	async renameFileSlug(oldSlug, newSlug, updateReferences) {
		...
		if (!fileInfo) throw new Error(ErrorMessage.FILE_NOT_FOUND);
		if (!isValidSlug(newSlug)) throw new Error(ErrorMessage.INVALID_SLUG);
		const extension = getExtensionFromFilename(fileInfo.filename);
		if (typeof extension === "string" && !newSlug.endsWith(`.${extension}`)) throw new Error(ErrorMessage.INVALID_SLUG);
		if (graph.notes.has(newSlug) || graph.aliases.has(newSlug) || graph.files.has(newSlug)) throw new Error(ErrorMessage.SLUG_EXISTS);
		await this.#io.moveArbitraryGraphFile(oldSlug, newSlug);
		...
		if (updateReferences) {
			for (const [noteSlug, outgoingLinks] of graph.indexes.outgoingLinks) if (outgoingLinks.has(oldSlug)) {
				notesThatNeedUpdate.add(noteSlug);
				const note = graph.notes.get(noteSlug);
				if (!note) throw new Error("Note from index is undefined. This should not happen.");
				const newBlocks = changeSlugReferencesInNote(graph.indexes.blocks.get(noteSlug), oldSlug, newSlug, newSlug);
				note.content = serialize(newBlocks);
				graph.indexes.blocks.set(note.meta.slug, newBlocks);
				updateIndexes(graph, note);
			}
		}
		await this.#io.flushChanges(graph, false, notesThatNeedUpdate, /* @__PURE__ */ new Set(), new Set([oldSlug, newSlug]));
		return fileInfo;
	}
	async deleteFile(slug) {
		slug = slug.normalize("NFC");
		const graph = await this.#io.getGraph();
		if (!graph.files.has(slug)) throw new Error(ErrorMessage.FILE_NOT_FOUND);
		const agfPath = getArbitraryFilePath(graph.files.get(slug));
	...
	async getSlugsOfDanglingFiles() {
		const graph = await this.#io.getGraph();
		const allUsedFileSlugs = getSlugsFromInlineText(getAllInlineSpans(Array.from(graph.indexes.blocks.values()).flat())).filter(isValidSlug);
	...
	async getReadableArbitraryGraphFileStream(slug, range) {
		...
		return await this.#io.getReadableArbitraryGraphFileStream(slug, filename, range);
	}
	async getFileInfo(slug) {
		slug = slug.normalize("NFC");
		const fileInfo = (await this.#io.getGraph()).files.get(slug);
		if (!fileInfo) throw new Error(ErrorMessage.FILE_NOT_FOUND);
		return fileInfo;
	}
	async getPins() {
		const graph = await this.#io.getGraph();
		return (await Promise.allSettled(graph.pinnedNotes.map((slug) => {
	...
	async pin(slug) {
		...
		graph.pinnedNotes = Array.from(new Set([...graph.pinnedNotes, slug]));
		const updatePins = oldLength !== graph.pinnedNotes.length;
		await this.#io.flushChanges(graph, updatePins, /* @__PURE__ */ new Set(), /* @__PURE__ */ new Set(), /* @__PURE__ */ new Set());
		return this.getPins();
	}
	async movePinPosition(slug, offset) {
		...
		if (!oldPins.includes(slug)) throw new Error(ErrorMessage.PINNED_NOTE_NOT_FOUND);
		const oldIndex = oldPins.indexOf(slug);
		const newIndex = oldIndex + offset;
		graph.pinnedNotes = oldPins.toSpliced(oldIndex, 1).toSpliced(newIndex, 0, slug);
	...
	async unpin(slugToRemove) {
		...
		graph.pinnedNotes = graph.pinnedNotes.filter((s) => {
			if (s === slugToRemove) updatePins = true;
...
//#endregion
//#region src/lib/notes-worker/index.ts
let notesProvider = null;
function getTransferables(value) {
	if (value instanceof ReadableStream) return [value];
	return [];
}
async function handleRPCCall(msg, respond) {
	const { id, method, args } = msg;
	if (!notesProvider) {
		respond({
			id,
			error: "NotesProvider not initialized"
		});
		return;
	}
	try {
...
function setupPortHandler(port) {
	port.onmessage = async (event) => {
		await handleRPCCall(event.data, (data, transfer) => port.postMessage(data, transfer ?? []));
	};
}
onmessage = async (event) => {
	...
	if ("action" in eventData) {
		if (eventData.action === "initialize") {
			let dirHandle;
			if (eventData.useOPFS) dirHandle = await navigator.storage.getDirectory();
			else if (eventData.folderHandle) dirHandle = eventData.folderHandle;
			else {
				postMessage({
					action: "error",
			...
			if (eventData.createDummyNotes) for (let i = 1; i <= 1e3; i++) await storageProvider.writeObject("note-" + i + ".subtext", "Test note " + i);
	...
	await handleRPCCall(eventData, (data, transfer) => postMessage(data, { transfer: transfer ?? [] }));
};
//#endregion
